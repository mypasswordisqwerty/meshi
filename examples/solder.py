...
box = product.box.SimpleBox(w, 40, l, pins={'height': 30}, bottom_thick=2)

# front
Cube(45.5, 10, 26.5, at={'x': w/2-28, 'y': -l/2}).substractFrom(box.front.panel)
Cube(11.5, 10, 29, at={'x': w/2-60, 'y': -l/2}).substractFrom(box.front.panel)

cs = -w/2+68
Cylinder(8, 10, at={'x': cs, 'y': -l/2}, rot={'x': 90}).substractFrom(box.front.panel)
Cube(25, 10, 15, at={'x': cs-26.75, 'y': -l/2}).substractFrom(box.front.panel)
...
Cube(25, 10, 12, at={'x': -w/2, 'y': -l/2}).substractFrom(box.front.panel)

# back
hp = w/2-30
Cube(11, 10, 9, at={'x': hp, 'y': l/2}).substractFrom(box.back.panel)
box.back.add(Cube(2, 5.5, 5, at={'x': hp-6.5, 'y': l/2-3.75}))
box.back.add(Cube(2, 5.5, 5, at={'x': hp+6.5, 'y': l/2-3.75}))
